...
class Pi0FAST(_model.BaseModel):
    ...
    def sample_actions(
        ...
        *,
        ...
        # pre-logit layer activations as embeddings
        pre_logits_acc = jnp.zeros((batch_size, max_decoding_steps+1, self.PaliGemma.llm.module.width), dtype=last_logit.dtype)
        pre_logits_acc = pre_logits_acc.at[:, 0, :].set(aux_out["pre_logits"][:, -1, :])

        def step(carry):
            rng, last_logit, output_tokens, cache, _, step, acc = carry
            ...
            acc = acc.at[:, step + 1, :].set(out["pre_logits"][:, 0, :])

            return rng, last_logit, output_tokens, kv_cache, all_eos, step + 1, acc
        ...
        _, _, output_tokens, _, _, decode_steps, decode_pre_logits = jax.lax.while_loop(
            cond, step, (rng, last_logit, output_tokens, kv_cache, False, 0, pre_logits_acc)
        )

        # average across decoding steps to get embeddings
        decode_mask = (jnp.arange(max_decoding_steps+1) < decode_steps)[None, :, None]
        pre_logit_embeddings = jnp.mean(decode_pre_logits, where=decode_mask, axis=1)

        return output_tokens, pre_logit_embeddings
